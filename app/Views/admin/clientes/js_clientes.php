...
<script type="text/javascript">
    ...
    {
        $.get('<?= base_url(Strtolower($entidade) . '/exibir') ?>', function(data)
        {
            MyTable.fnDestroy();
    ...
    $(document).on("click", ".editar-dados<?= $entidade ?>", function() {
        ...
        $.ajax({
            method: "POST",
            url: "<?= base_url(Strtolower($entidade) . '/editar') ?>",
            data: "id=" + id + '&tp=1'
        })
        .done(function(data) {
    ...
    $(document).on("click", ".confirmarExcluir-<?= Strtolower($entidade) ?>", function() {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    method: "POST",
                    url: "<?= base_url(Strtolower($entidade) . '/excluir') ?>",
                    data: "id=" + id_objeto
                })
                .done(function(data) {
                    if (data == '0') {
    ...
    $('#form-inserir-<?= Strtolower($entidade) ?>').submit(function(e) {
        ...
        $.ajax({
            method: 'POST',
            url: '<?= base_url(Strtolower($entidade) . '/salvar') ?>',
            data: data
        })
        .done(function(data) {
    ...
    $(document).on('submit', '#form-editar-<?= Strtolower($entidade) ?>', function(e){
        ...
        $.ajax({
            method: 'POST',
            url: '<?php echo base_url(Strtolower($entidade) . '/salvar'); ?>',
            data: data
        })
        .done(function(data) {
            var out = jQuery.parseJSON(data);
